...
function setup() {
  // create canvas sized to fill remaining space
  ...
  // recompute on slider change and reset animation
...
function computeSolution() {
  ...
  for (let i = 1; i <= steps; i++) {
    const t = i * dt;
    const x = preyCurve[i - 1];
    const y = predCurve[i - 1];
    // RK4 integration
...
function draw() {
  ...
  stroke(0);
  line(map(tValues[idx], 0, tmax, 0, w), 0, map(tValues[idx], 0, tmax, 0, w), h);
  pop();

  // Bottom: agent-based view
  push(); translate(0, h);
  noStroke(); fill(0, 77, 0); rect(0, 0, w, h);
  const nPrey = floor(preyCurve[idx]);
  const nPred = floor(predCurve[idx]);
  fill(preyColor);
